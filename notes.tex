...
\begin{theorem}[The Generalized Mean Value Theorem]
  Let $f,g: [a,b] \to \R$ be continuous on the interval and differentiable in $(a,b)$. Then $\exists x\in
  (a,b)$ such that $$f'(x)\big[(g(b) - g(a)\big] = g'(x)\big[f(b) - f(a)\big].$$
\end{theorem}
\begin{proof}
  Consider the function $h: [a,b] \to \R$ defined by
  $$h(t) = f(t)\big[(g(b) - g(a)\big] - g(t)\big[f(b) - f(a)\big].$$
  We must find $x\in (a,b)$ such that $h'(x) = 0$. Observe that $h$ is continuous on $[a,b]$ and
  differentiable on $(a,b)$. Since $[a,b]$ is compact, $h$ attains a maximum and minimum value on the
  interval. Now use the fact that $h(a) = h(b)$.
  If $h$ is constant, any $x\in (a,b)$ satisfies $h'(x) = 0$. If $\exists t\in (a,b)$ such that
  $h(t) > h(a)$, then the maximum is attained at some $x\in (a,b)$, and $h'(x) = 0$. The analogous
  argument for a minimum also applies.
\end{proof}
...
\end{corollary}
\begin{proof}
  Take $g$ to be the identity function above.
\end{proof}
...
\begin{defn}[The Riemann Integral]
...
\begin{theorem}[Continuous function of an integrable function is integrable]
...
\begin{theorem}[Properties of Integration]\leavevmode
 \begin{enumerate}
   \item \emph{linearity}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    %&= |\gamma'(x_i)|(x_i - x_{i-1}| + \frac{\varepsilon}{2}(x_i - x_{i-1}) \\
...
\begin{theorem}[``Cauchy'' Condition]
